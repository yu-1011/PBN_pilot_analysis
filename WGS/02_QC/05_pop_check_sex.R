...
# Read in data
imp_sex <- read.table(paste0(pop,"_",batch,"_btqc_mgqc-chrx.sexcheck"), header=T)

imp_sex$PEDSEX <- factor(imp_sex$PEDSEX, levels = c(1, 2, 0, -9))
levels(imp_sex$PEDSEX) <- c("Male", "Female", "Unknown", "Unknown")
imp_sex$SNPSEX <- factor(imp_sex$SNPSEX, levels = c(1, 2, 0, -9))
levels(imp_sex$SNPSEX) <- c("Male", "Female", "Unknown", "Unknown")
...
p <- ggplot(imp_sex, aes(x=F, fill=SNPSEX)) +
  ...
  labs(x='F-statistic',y='Frequency',
       ...
       fill='Imputed sex') +
 	   scale_fill_manual(values = c("#56B4E9", "#FD8D3C","grey30"), labels=c( "Male", "Female","Unknown"))
ggsave(paste0(pop,"_",batch,"_sexcheck_fstat_F02_M08.pdf"), p, width=7.5, height=4)


# Plot: imputed vs. reported gender
pdf(paste0(pop,"_",batch,"_sexcheck_fstat_F02_M08_ped.vs.snpsex.pdf"), width=7.5, height=5)
ggplot(imp_sex, aes(y=F,x=factor(SNPSEX), color=factor(SNPSEX))) +
  geom_jitter(alpha=0.7) +
  labs(x='Imputed gender',y='chrX F-statistic',
       title=paste0(toupper(pop),"_", batch, ' sample sex check'),
       color='Imputed gender') +
  scale_color_manual(values = c("#56B4E9","#FD8D3C","grey30")) +
  theme_bw()
ggplot(imp_sex, aes(y=F,x=factor(SNPSEX), color=factor(PEDSEX))) +
  geom_jitter(alpha=0.7) +
  labs(x='Imputed gender',y='chrX F-statistic',
       title=paste0(toupper(pop),"_", batch, ' sample sex check'),
       color='Self-reported gender') +
  scale_color_manual(values = c("#56B4E9","#FD8D3C","grey30")) +
  theme_bw()
dev.off()
...
write.table(sex_mismatch_F02_M08_inds, paste0(pop,"_",batch,"_sex_mismatch_F02_M08.indlist"), quote=F, col.names=F, row.names=F)
write.table(sex_mismatch_F025_M075_inds, paste0(pop,"_",batch,"_sex_mismatch_F025_M075.indlist"), quote=F, col.names=F, row.names=F)
write.table(sex_mismatch_F03_M07_inds, paste0(pop,"_",batch,"_sex_mismatch_F03_M07.indlist"), quote=F, col.names=F, row.names=F)
write.table(sex_mismatch_F04_M06_inds, paste0(pop,"_",batch,"_sex_mismatch_F04_M06.indlist"), quote=F, col.names=F, row.names=F)
write.table(sex_mismatch_F05_M05_inds, paste0(pop,"_",batch,"_sex_mismatch_F05_M05.indlist"), quote=F, col.names=F, row.names=F)
