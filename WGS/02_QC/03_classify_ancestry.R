...
print("Read in data")
# Read in data
pca_w_ref <- read.table(paste0(batch,"_ref.pca.eigenvec"), header=T, sep="\t", stringsAsFactors=F)
pca_w_ref$pop <- pca_w_ref$FID


pca_w_ref$superpop <- batch
# if ( refname == "1kg" ) {
...
pca_w_ref$superpop <- factor(pca_w_ref$superpop,
                              levels=c(batch,"AFR","AMR","EAS","EUR","SAS"))

# }


print(paste0("Make PC plots: ", batch, " + ref samples, colored by super population"))
# PC plots: batch + ref samples, colored by super population
if ( pcplot0 == "TRUE" ){
    for(i in 1:5){
        ...
        labs(x=paste0('PC',i), y=paste0('PC',i+1), title=paste0(batch," + 1KG samples"),
        color="Super population")
    # print(p)
        ggsave(paste0(batch,"_ref.PC",i,"_PC",i+1,"_by_superpop.pdf"), p, width=7.5, height=6)
    }
}


# # Plot: batch-seq samples in ref space
# pca_w_ref.batch <- subset(pca_w_ref, superpop=="batch")
# for(i in 1:5){
#   p = ggplot(pca_w_ref.batch, aes_string(x=paste0('PC',i), y=paste0('PC',i+1))) +
#     geom_point(aes(color=superpop), alpha=0.6, size=0.95) +
#     scale_color_manual(values = pal_d3("category20")(20)[1]) + 
#     theme_bw() +
#     guides(color = guide_legend(override.aes = list(size=2))) +
#     labs(x=paste0('PC',i), y=paste0('PC',i+1), title="batch samples (in 1KG space)", 
#     	color="Study sample    ")
#   # print(p)
#   ggsave(paste0("batch_ref.PC",i,"_PC",i+1,"_by_studysample.pdf"),
...
pca_w_ref.batch <- subset(pca_w_ref, superpop==batch)
pca_w_ref.pred <- merge(pca_w_ref.batch, pop_pred, by="IID")
pca_w_ref.pred$predicted_pop <- factor(pca_w_ref.pred$predicted_pop, levels=c("AFR","AMR","EAS","EUR","SAS"))


print(paste0("Make PC plots: ", batch, " samples only, colored by predicted super population/ancestry"))
# PC plots: batch samples only, colored by predicted super population/ancestry
for(i in 1:5){
  p = ggplot(pca_w_ref.pred, aes_string(x=paste0('PC',i), y=paste0('PC',i+1))) +
    ...
    theme_bw() +
    guides(color = guide_legend(override.aes = list(size=2))) +
    labs(x=paste0('PC',i), y=paste0('PC',i+1), title=paste0(batch," samples (in 1KG space)"),
         color="Predicted population")
  # print(p)
  ggsave(paste0(batch,"_ref.PC",i,"_PC",i+1,"_by_predPop.pdf"),
         p, width=7.75, height=6)
}


print(paste0("Make PC plots: ",batch," samples only, colored by predicted superpop w. a pred. prob > ",pred_prob))
# PC plots: batch samples only, colored by predicted super population w. a pred. prob > pred_prob
...
for(i in 1:5){
  p = ggplot(pca_w_ref.pred.sub, aes_string(x=paste0('PC',i), y=paste0('PC',i+1))) +
    ...
    labs(x=paste0('PC',i), y=paste0('PC',i+1), title=paste0(batch, " samples (in 1KG space)"),
         color="Predicted population")
  # print(p)
  ggsave(paste0(batch, "_ref.PC",i,"_PC",i+1,"_by_predPop",pred_prob,".pdf"),
         p, width=7.75, height=6)
}
...
write.table(pca_w_ref.pred, paste0(batch, "_ref.PC.predPop.tsv"), quote=F,
  col.names=T, row.names=F, sep="\t")

# eur_inds <- pca_w_ref.pred.sub[pca_w_ref.pred.sub$predicted_pop=="EUR", c("FID","IID")]
# write.table(eur_inds, paste0("batch_ref.PC.predPop",pred_prob,".EUR.indlist"), quote=F,
