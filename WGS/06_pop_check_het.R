...
# Read in data
het <- read.table(paste0(pop, "_", batch, "_btqc_mgqc-inbr.hetrate"), header=T)
...
ggplot(het, aes(F)) +
  ...
  geom_vline(xintercept=0.2, linetype="dashed") +
  theme_bw()
ggsave(paste0(pop, "_", batch, "_hetcheck_fstat_thresh02.pdf"), width=7, height=4)
...
ggplot(het, aes(HetRate)) +
  ...
  theme_bw()
ggsave(paste0(pop,"_", batch, "_hetcheck_hetrate_dev_from_mean.pdf"), width=7, height=4)
...
write.table(het_fstat_outliers, paste0(pop,"_",batch,"_het_outlier_f02.indlist"), quote=F, col.names=F, row.names=F)
...
write.table(hetrate_cutoff1_outliers, paste0(pop, "_", batch, "_het_outlier_3sd.indlist"), quote=F, col.names=F, row.names=F)
write.table(hetrate_cutoff2_outliers, paste0(pop, "_", batch, "_het_outlier_5sd.indlist"), quote=F, col.names=F, row.names=F)
