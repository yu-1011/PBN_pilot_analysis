...
# Read in data
ibd <- read.table(gzfile(paste0(pop,"_",batch,"_btqc_mgqc-ibd-min0.125.genome.gz")), header=T)
...
p <- ggplot(ibd, aes(Z0, Z1,color=inferred_rel)) +
  ...
  theme_bw()
ggsave(paste0(pop,"_",batch,"_ibd_Z0.vs.Z1.pdf"), p, width=6.8, height=5)
...
write.table(ibd_rel_inds_remove, paste0(pop,"_",batch,"_ibd_pihat02.indlist"), quote=F, col.names=F, row.names=F)
